...
export default function Header() {
    ...
    const changeLanguage = (lg) => {
        ...
        reset();
        startTransition(() => {
            router.push(searchParams.toString() ? `${newUrl}?${searchParams.toString()}` : newUrl);
        })
    ...
    return (
        <header className={`${s.header} ${burgerMenu ? s.active : ''} ${pathname == '/' + locale ? s.fixed__bg : ''} ${blurBg && pathname == '/' + locale ? s.blur__bg : ''}`}>
            ...
            <nav className='container'>
                ...
                <ul>
                    ...
                    <li><Link href='#' onClick={() => setBurgerMenu(!burgerMenu)}>
                    {t('header.productsPage')}
                     </Link></li>
                    <li
                    className={`${isActive('projects') ? s.activeNav : ''}`}
                    ><Link href='/projects' onClick={() => setBurgerMenu(!burgerMenu)}>
                    {t('header.projectsPage')}       
                    </Link></li>
                    <li
                    className={`${isActive('laboratory') || isActive('certificates')  ? s.activeNav : ''}`}
                    >
                        <span>
                        {t('header.arg.argPage')}
                        ...
                        <ul className={s.submenu}>
                            <li><Link href='/laboratory' onClick={() => setBurgerMenu(!burgerMenu)}>
                            {t('header.arg.laboratoryPage')}
                            </Link></li>
                            <li><Link href='/certificates' onClick={() => setBurgerMenu(!burgerMenu)}>
                            {t('header.arg.certificatesPage')}
                            </Link></li>
                        </ul>
                    </li>
                    ...
                    <li
                    className={`${isActive('news') ? s.activeNav : ''}`}
                    ><Link href='/news' onClick={() => setBurgerMenu(!burgerMenu)}>
                    {t('header.newsPage')}
                    </Link></li>
                    <li
                    className={`${isActive('blogs') ? s.activeNav : ''}`}
                    ><Link href='/blogs' onClick={() => setBurgerMenu(!burgerMenu)}>
                    {t('header.blogsPage')}

                    </Link></li>
                    <li
                     className={`${isActive('about') ? s.activeNav : ''}`}
                    ><Link href='/about' onClick={() => setBurgerMenu(!burgerMenu)}>
                    {t('header.aboutPage')}
                    ...
                    <li className={s.langs}>
                        ...
                        <ul className={s.other__langs}>
                            {
                                locales.map(item => {
                                    if(locale !== item){
                                        return(<li>
                                            <button onClick={() => changeLanguage(item)}>
                                                {item.toUpperCase()}
                                            </button>
                                        </li>)
                                    }
